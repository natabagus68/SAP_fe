...
import { PreventiveApiRepository } from "@data/api/report/preventive-api-repository";
...
export default function usePreventive() {
  ...
  const [dataPreventiveById, setDataPreventiveById] = useState(null);
  ...
  const onOpenDetail = (data): void => {
    navigate("details", {
      state: {
        data: {
          id: dataPreventiveById?.id,
          idPreventive: dataPreventiveById?.idPreventive,
          date: dataPreventiveById?.date,
          noMesin: dataPreventiveById?.noMesin,
          pelaksana: dataPreventiveById?.pelaksana,
        },
      },
    });
  };
  ...
  //get data preventive by Id
  const getDataPreventiveById = async (id: string) => {
    try {
